...
recenter = {
    ...
    for pos_vel in [("xyz", "cops"), ("vxyz", "vcents")]
    for t in T
}

coord_type = {
    "{:s}_{:s}".format(pos_vel[0], t): pos_vel[1]
    for pos_vel in [("xyz", "position"), ("vxyz", "velocity")]
    for t in T
}
...
def particle_mask_fof(ptype):
    ...
    def mask(obj_id, vals=None, **kwargs):
        ...
        lasts = np.cumsum(vals["l_" + ptype], dtype=int).value
...
def particle_mask_fofsub(ptype):
    ...
    def mask(obj_id, vals=None, masks=None, **kwargs):
        fmask = fof_mask(obj_id, vals=vals, **kwargs)
        gmask = masks["group"]
        fof_offs = np.cumsum(vals["l_" + ptype], dtype=int).value
        fof_offs = np.r_[0, fof_offs[:-1]]
        fof_off = fof_offs[fmask][0]
        isub = np.argmax(gmask)
        sub_off = np.sum(vals["sl_" + ptype][isub - obj_id.sub : isub], dtype=int)
        sub_len = np.array(vals["sl_" + ptype], dtype=int)[gmask][0, 0]
...
def particle_mask_aperture(ptype):
    ...
    def mask(obj_id, vals=None, aperture=None, masks=None, **kwargs):
        ...
        retval[need_D] = np.sum(np.power(vals[key][need_D], 2), axis=1) < np.power(
            aperture, 2
        )
        return retval
...
def group_mask(obj_id, vals=None, **kwargs):
    ...
    return (np.array([(vals.firstsub[fmask] + obj_id.sub).value], dtype=int),)


@usevals(("nfof",))
def fof_mask(obj_id, vals=None, **kwargs):
    return (np.array([obj_id.fof], dtype=int),)
